...
#define COPY_COM_SERVICE_UUID(uuid_struct)	COPY_UUID_128(uuid_struct,0x02,0x36,0x6e,0x80, 0xcf,0x3a, 0x11,0xe1, 0x9a,0xb4, 0x00,0x02,0xa5,0xd5,0xc5,0x1b)
#define COPY_COM_SEND_UUID(uuid_struct)		COPY_UUID_128(uuid_struct,0xe2,0x3e,0x78,0xa0, 0xcf,0x4a, 0x11,0xe1, 0x8f,0xfc, 0x00,0x02,0xa5,0xd5,0xc5,0x1b)
#define COPY_CONF_SERVICE_UUID(uuid_struct) COPY_UUID_128(uuid_struct,0x34,0x0a,0x1b,0x80, 0xcf,0x4b, 0x11,0xe1, 0xac,0x36, 0x00,0x02,0xa5,0xd5,0xc5,0x1b)
#define COPY_CONF_SEND_UUID(uuid_struct)	COPY_UUID_128(uuid_struct,0x42,0x82,0x1a,0x40, 0xe4,0x77, 0x11,0xe2, 0x82,0xd0, 0x00,0x02,0xa5,0xd5,0xc5,0x1b)
...
uint16_t comServHandle, comSendCharHandle;
uint16_t confServHandle, confSendCharHandle;
...
/*******************************************************************************
 * Function Name  : Add_Com_Service
 * Description    : Add the 'Communication' service. Analog zu Add_Acc_Service.
 * Input          : None
 * Return         : Status.
 *******************************************************************************/
tBleStatus Add_Com_Service(void)
{
	...
	COPY_COM_SERVICE_UUID(uuid);

	Osal_MemCpy(&service_uuid.Service_UUID_128, uuid, 16);
	ret = aci_gatt_add_service(UUID_TYPE_128, &service_uuid, PRIMARY_SERVICE,
			Services_Max_Attribute_Records[ACCELERATION_SERVICE_INDEX],	//aktuell 7
			&comServHandle);
	if (ret != BLE_STATUS_SUCCESS)
		goto fail;

	COPY_COM_SEND_UUID(uuid);
	Osal_MemCpy(&char_uuid.Char_UUID_128, uuid, 16);
	ret = aci_gatt_add_char(comServHandle, UUID_TYPE_128, &char_uuid, 6,
	CHAR_PROP_NOTIFY | CHAR_PROP_READ, ATTR_PERMISSION_NONE,
	GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP, 16, 0, &comSendCharHandle);
	if (ret != BLE_STATUS_SUCCESS)
		goto fail;

	PRINTF("Service COM added. Handle 0x%04X, Send Charac handle: 0x%04X\n",comServHandle, comSendCharHandle);
	return BLE_STATUS_SUCCESS;

	fail: PRINTF("Error while adding COM service.\n");
	return BLE_STATUS_ERROR;
}

/*******************************************************************************
 * Function Name  : Add_Conf_Service
 * Description    : Add the 'Configuration' service. Analog zu Add_Acc_Service.
 * Input          : None
 * Return         : Status.
 *******************************************************************************/
tBleStatus Add_Conf_Service(void)
{
	...
	COPY_CONF_SEND_UUID(uuid);
	Osal_MemCpy(&char_uuid.Char_UUID_128, uuid, 16);
	ret = aci_gatt_add_char(comServHandle, UUID_TYPE_128, &char_uuid, 6,
			CHAR_PROP_NOTIFY | CHAR_PROP_READ, ATTR_PERMISSION_NONE,
	GATT_NOTIFY_READ_REQ_AND_WAIT_FOR_APPL_RESP, 16, 0, &confSendCharHandle);
	if (ret != BLE_STATUS_SUCCESS)
		goto fail;

	PRINTF("Service CONF added. Handle 0x%04X, Send Charac handle: 0x%04X\n",confServHandle, confSendCharHandle);
	return BLE_STATUS_SUCCESS;

	fail: PRINTF("Error while adding CONF service.\n");
	return BLE_STATUS_ERROR;
}
