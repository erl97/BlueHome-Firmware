...
/* Set NUM_BLOCKS_X_PACKET used on OTA FW data transfer: OTA packet actual size = 4 bytes (OTA sequence number, check sum needs ack) + (NUM_BLOCKS_X_PACKET * BYTE_INCREMENT) bytes in a single data packet */
#define NUM_BLOCKS_X_PACKET  ((OTA_ATT_MTU_SIZE-4)/(BYTE_INCREMENT)) 
    
/* Flash burst write guard time for BYTE_INCREMENT (16) * NUM_BLOCKS_X_PACKET  * NOTIFICATION_WINDOW (8 writes) bytes */
#define OTA_WRITE_GUARD_TIME (ceil((((((((4 + (NUM_BLOCKS_X_PACKET * 16)) * NOTIFICATION_WINDOW)) / NUM_BURST_WRITE_BYTES)) * BURST_WRITE_BYTES_uS_TIME) /((float)1000))))

/* Extended packet lenght: L2CAP connection interval for setting proper connection interval vs flash write operations */
...
#define OTA_EXT_LEN_L2CAP_CONN_INTERVAL  (ceil((OTA_WRITE_GUARD_TIME + 3.5)/1.25)) //Added further 3.5 ms margin for BLE communication
    
#endif  

/* OTA FW data packet size */
#define DATA_PACKET_SIZE (BYTE_INCREMENT * NUM_BLOCKS_X_PACKET)
#define IMAGE_CONTENT_SIZE (DATA_PACKET_SIZE + 4) /* + 4 for (OTA sequence number, check sum needs ack) */

#define SEQ_NUM_HIGH_INDEX (IMAGE_CONTENT_SIZE - 1) /* sequence number high index */
#define SEQ_NUM_LOW_INDEX  (IMAGE_CONTENT_SIZE - 2) /* sequence number low index */
#define NEEDS_ACK_INDEX    (IMAGE_CONTENT_SIZE - 3) /* needs ack index */

/* buffer size to hold only the image file data on received notification/s and to be written on flash */
#define BUF_SIZE (DATA_PACKET_SIZE * NOTIFICATION_WINDOW)  
                          ...
                          uint8_t *att_data)
{
    ...
    if (attr_handle == (btlNewImageCharHandle + 1)){
       ...
       * Get base_address and image size + notification range rrquested from client.
       */
       ...
       /* Here we read updated characteristic content filled by "write with no response command' coming from the master */
          ...
          */
          ...
          if (checksum == att_data[0]){
             ...
             receivedSeqNum = ((att_data[SEQ_NUM_HIGH_INDEX]<<8) + att_data[SEQ_NUM_LOW_INDEX]);
             ...
             { 
               ...
               if ((att_data[NEEDS_ACK_INDEX] == 1) || (((receivedSeqNum+1)*DATA_PACKET_SIZE) >= imageSize)) //TBR ((receivedSeqNum+1) % notification_range) or (att_data[NEEDS_ACK_INDEX] == 1)
               { 
                                     ...
                                     uint16_t MaxRxTime)
{
    ...
    aci_l2cap_connection_parameter_update_req(Connection_Handle, (uint16_t) OTA_EXT_LEN_L2CAP_CONN_INTERVAL, (uint16_t) OTA_EXT_LEN_L2CAP_CONN_INTERVAL, 0, 0xC80); 
}
#endif 
